...
final class DataBaseRepository_Tests: XCTestCase {
    ...
    private var repository: DataBaseRepository? = DependenciesManager.Repository.dataBaseRepository
...
extension DataBaseRepository_Tests {
    ...
    func test_trackedEntityDeleteAll() {
        ...
        XCTAssertEqual(repository?.trackedEntityGetAll(cascade: false).count ?? 0, 0)
        ...
        XCTAssertEqual(repository?.trackedEntityGetAll(cascade: false).count ?? 0, 2)
     ...
     func test_trackedEntitySaveAndRetrieved() {
         ...
         let before = repository?.trackedEntityGetAll(cascade: false).count ?? 0
         ...
         let after = repository?.trackedEntityGetAll(cascade: false).count ?? 0
